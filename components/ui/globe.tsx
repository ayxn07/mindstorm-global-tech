...
export function World(props: WorldProps) {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      (entries) => {
        setFrameloop(entries[0]?.isIntersecting ? "always" : "demand");
      },
      { rootMargin: "160px" },
    );
    io.observe(node);
  ...
  const effectiveFrameloop = prefersReduced ? "demand" : frameloop;
  const autoRotate = prefersReduced ? false : (globeConfig.autoRotate ?? true);
